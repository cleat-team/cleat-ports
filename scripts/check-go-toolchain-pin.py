...
# `go-version: "1.26"` in any of the quoting styles the tree uses, including the
# flow-mapping form `with: { go-version: "1.26" }` that both call sites use.
PIN_RE = re.compile(r"""go-version:\s*['"]?(\d+(?:\.\d+)*)['"]?""")
...
def find_pins(root, workflows):
    """[(path, lineno, version)] for every go-version pin, and any read error."""
    pins = []
...
def check(root, workflows=WORKFLOWS, fetch=http_get, env=None, floor=None):
    ...
    out.append("pins checked: %d" % len(pins))
...
CASES = [
    # name,                       workflow text,                  floor,   want rc, want substring
    ("pin equals floor",          'with: { go-version: "1.26" }', "1.26.0", 0, "every pin is at or above"),
    ("pin above floor",           'with: { go-version: "1.27" }', "1.26.0", 0, "every pin is at or above"),
    ("pin below floor",           'with: { go-version: "1.25" }', "1.26.0", 1, "below the go 1.26.0"),
    ("the real cleat-ports#251",  'with: { go-version: "1.25" }', "1.26.0", 1, "GOTOOLCHAIN=local"),
    ("major below floor",         'with: { go-version: "1.9" }',  "1.26.0", 1, "below the go 1.26.0"),
    ("unquoted pin",              "go-version: 1.25",             "1.26.0", 1, "below the go 1.26.0"),
    ("patch pin satisfies",       'go-version: "1.26.1"',         "1.26.0", 0, "every pin is at or above"),
    ("patch pin below",           'go-version: "1.26.0"',         "1.26.1", 1, "below the go 1.26.1"),
    ("no pin at all",             "runs-on: ubuntu-latest",       "1.26.0", 2, "Zero pins agrees"),
]
...
def self_test():
    ...
    failures = []
    for name, text, floor, want_rc, want_sub in CASES:
    ...
    if failures:
        ...
        print("\n".join(failures))
        return 1
    print("OK: self-test passed, all %d cases classified correctly." % (len(CASES) + 2))
    return 0
