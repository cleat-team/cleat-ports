...
## `dbVisibilityPersistenceTest.go` closed — 12 cases, 2 issues

The largest of the five unopened files, and the one that made the case for
opening them. Every case adjudicated.

| case | verdict |
|---|---|
| `TestVisibilityPagination` | **gap → cleat#1182** |
| `TestFilteringByType` | **gap → cleat#1183** |
| `TestCronVisibility` | **gap → cleat#1183** |
| `TestFilteringByCloseStatus` | **gap → cleat#1183** (and see the correction below) |
| `TestFilteringByWorkflowID` | folded into #1183 — the same missing structured filter |
| `TestGetClosedExecution` | **covered** — `test_an_unknown_run_is_a_clean_404_on_every_read_path`, across five read paths |
| `TestBasicVisibility` | **covered** — `test_the_workflow_list_contains_a_run_that_was_started` |
| `TestBasicVisibilityTimeSkew` | not portable — both timestamps are database-generated by one server, so start/close inversion cannot occur |
| `TestClosedWithoutStarted` | not portable — the row is created at start; a close-without-start is unrepresentable |
| `TestDelete` | not portable — see below |
| `TestMultipleUpserts` | not portable — see below |
| `TestUpsertWorkflowExecution` | not portable — validation of an upsert API cleat does not have |

### The unifying reason for the five "not portable" rows

Cadence keeps **two stores**: the execution store and a separate visibility
store, kept in step by `RecordWorkflowExecutionStarted` / `…Closed` /
`UpsertWorkflowExecution`. Five of these twelve cases exist to test that the two
agree under awkward sequences —

- a close arriving with a timestamp *before* its start (`TimeSkew`);
- a close for a workflow the visibility store never saw start
  (`ClosedWithoutStarted`);
- a start re-recorded *after* a close, which must not erase the closed record
  (`MultipleUpserts`);
- a delete that must propagate (`TestDelete`);
- direct upsert validation (`TestUpsertWorkflowExecution`).

**cleat has one row.** There is no second store to disagree with, so every one
of those questions is unaskable rather than answered. That is a genuinely
different verdict from "cleat lacks this": the class of bug does not exist here.

It also means the file's portable content was concentrated in the four
*filtering and paging* cases — the ones that ask what an operator can retrieve
rather than whether two stores agree.

### A correction that belongs with the record

`TestFilteringByCloseStatus` was first triaged **covered**, on the grounds that
cleat has `filter.Status`. That is scoring a case from the API rather than the
behaviour — the error the `TestGetCurrentWorkflow` section above is entirely
about, repeated within the hour by the person who wrote it.

Re-checked: cleat has no `cancelled` status, so cancelled runs cannot be
selected as a class. And measured afterwards, they end as **`done`** — which
`cleat-ports` already asserts in
`test_a_cancelled_workflow_still_reports_status_done`, a test whose docstring
states the divergence in the same words the issue used. **The finding was
already in this repository**; it was reached from the cleat side without
searching the port for a test on the surface being described.

What survived that round is not in either place: a run that **honoured** a
cancel and one that **ignored** it produce byte-identical rows —
