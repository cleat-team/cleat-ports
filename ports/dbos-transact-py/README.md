...
| `test_listing_filters.py` | 5 | none — ported from uber/cadence's dbVisibilityPersistenceTest.go, not from DBOS: status, def_name, limit/offset, the half-open time window, and id_prefix |
...
| `test_observe_arbitrary_run.py` | 2 | `test_dbos.py` — upstream test_retrieve_workflow_in_workflow -- reading a run you did not spawn, which poll_child has always allowed despite its name (ISSUES 33 withdrawn, cleat#1120) |
...
| `test_signals.py` | 5 | `test_dbos.py` — `recv` with a timeout, and `send` between workflows |
| `test_tenant_isolation.py` | 3 | none — cleat-specific: one tenant cannot read another's runs, which needs the second tenant #210 provisions |
